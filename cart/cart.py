...
class Cart(object):
    def __init__(self, request):
        ...
        if not cart:
            # salva um carrionho vazio na sessão
            cart = self.session[settings.CART_SESSION_ID] = {}
        self.cart = cart

    def add(self, product, quantity=1, override_quantity=False):
        """
        Adiciona um produto no carrionho de compras ou atualiza a sua quantidade
        """
    ...
    # esse método permitirrá iterar pelos itens do carrinho nas views e nos templates
    def __iter__(self):
        """
        Itera pelos itend do carrinho de compras e obtém
